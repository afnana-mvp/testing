...
interface CplusProps {}

const Cplus: FC<CplusProps> = ({}) => {
    ...
    <div className="bg-background_color  flex items-center justify-center flex-col gap-[50px] text-center">
      <div className="flex items-center w-full justify-center bg-[url('../../public/coding/java_tool/java_main.png')]  bg-cover bg-center	bg-no-repeat h-[50vh] lg:h-[90vh] ">
        <h1 className="text-white text-center text-2xl sm:text-4xl md:text-5xl lg:text-6xl font-semibold !leading-relaxed ">
          {t("cplusimageText")} <br /> {t("language by")} <span className="araby_ai"> ArabyAi</span>
        </h1>
      </div>
      <div className="flex items-center  justify-center gap-10 flex-col md:flex-row w-5/6 ">
        <Image src={cplusIcon} alt="python image" width={100} className="w-20 md:w-32" />
        <p className="text-[12px] md:text-xl basis-9/12	md:text-left md:rtl:text-right">{t("cplusiconText")}</p>
      </div>
      ...
      <div className="flex items-center justify-center  container flex-col md:flex-row  md:w-2/3 gap-10 md:gap-0 ">
        <p className="mx-8 text-[12px] md:text-xl md:text-left basis-1/2 md:rtl:text-right">{t("cplushelpText")}</p>
      ...
      <div className="flex container  flex-col-reverse md:flex-row items-center justify-center  relative  gap-10 md:gap-36 pb-20 ">
        <Image src={cplusCode} alt="python image" className="w-80 md:w-96  object-cover z-10" />
